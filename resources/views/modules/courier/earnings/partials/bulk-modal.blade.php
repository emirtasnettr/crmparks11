...
>
    ...
    <div x-show="activeModal === 'bulk'" x-transition class="relative w-full max-w-lg rounded-xl border border-gray-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Toplu Hakediş Oluştur</h3>
        ...
        <div class="space-y-4 px-6 py-4">
            ...
            <x-ui.file-upload name="bulk_file" label="Excel Dosyası Seç" accept=".xlsx,.xls,.csv" />

            <div x-show="bulkSaved" x-cloak>
                <x-ui.alert type="success">Dosya doğrulandı. Toplu hakediş oluşturma backend bağlantısı sonrası aktif olacaktır.</x-ui.alert>
            </div>

            <div class="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
                <x-ui.button type="button" variant="secondary" @click="closeModals">İptal</x-ui.button>
                <x-ui.button type="button" @click="importBulk">İçe Aktar</x-ui.button>
            </div>
        </div>
    </div>
</div>
