...
<div x-data="agencyEarningPage(@js(['openBulk' => $errors->has('file')]))">
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">
                Acentelere ait hakediş ve ödeme kayıtlarını yönetin.
            </p>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <x-ui.export-button :href="route('agencies.earnings.export', request()->query())" />
        </div>
    </div>
    ...
    <x-ui.card :padding="false">
        <form method="GET" action="{{ route('agencies.earnings.index') }}" class="p-4 sm:p-6">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <x-ui.button href="{{ route('agencies.earnings.index') }}" variant="secondary">Temizle</x-ui.button>
            </div>
        </form>
    </x-ui.card>
                    ...
                    @forelse ($earnings as $earning)
                        <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-slate-800/50">
                            <td class="max-w-[200px] px-4 py-3 sm:px-6">
                                <p class="line-clamp-2 font-medium text-gray-900 dark:text-white">{{ $earning['agency_name'] }}</p>
                                <p class="mt-0.5 font-mono text-xs text-gray-400 dark:text-slate-500">{{ $earning['reference'] }}</p>
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-gray-900 dark:text-white">{{ $earning['period_label'] }}</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400">{{ $earning['period_type_label'] }}</p>
                            </td>
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">
                                {{ number_format($earning['courier_count']) }}
                            </td>
                            ...
                            <td class="px-4 py-3 sm:px-6">
                                <x-agency.earning-row-actions :earning="$earning" />
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    @include('modules.agency.earnings.partials.single-modal')
    @include('modules.agency.earnings.partials.bulk-modal')
</div>
@endsection
