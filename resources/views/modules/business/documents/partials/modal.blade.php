<x-ui.modal title="Evrak Yükle">
    <form
        method="POST"
        action="{{ $formAction ?? route('businesses.documents.store') }}"
    ...
    >
        ...
        <div class="space-y-1.5">
            ...
            >
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">PDF, Word, Excel, Resim veya ZIP (maks. {{ config('crmlog.upload.max_size_mb') }}MB)</p>
                <input
                    type="file"
                    name="file"
                    class="sr-only"
