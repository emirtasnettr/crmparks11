...
>
    ...
    <div x-show="activeModal === 'single'" x-transition class="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
        <div class="sticky top-0 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-slate-700 dark:bg-slate-800">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white" x-text="editId ? 'Hakediş Düzenle' : 'Tekli Hakediş'"></h3>
        ...
        <div class="px-6 py-4">
            <form
                method="POST"
                :action="editId ? `${routes.update}/${editId}` : routes.store"
                @submit="handleSingleSubmit($event)"
                class="space-y-4"
            >
                @csrf
                ...
                <div class="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
                    ...
                    <x-ui.button type="submit" ::disabled="submitting" x-text="editId ? 'Güncelle' : 'Kaydet'"></x-ui.button>
                </div>
            </form>
        </div>
    </div>
