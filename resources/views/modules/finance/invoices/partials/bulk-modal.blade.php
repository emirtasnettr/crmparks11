    ...
    <div x-show="activeModal === 'bulk'" x-transition class="relative w-full max-w-lg rounded-xl border border-gray-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
        ...
        <form @submit.prevent="saveBulk()" class="space-y-4 px-6 py-4">
            <p class="text-sm text-gray-600 dark:text-slate-300">
                Onaylanmış hakedişlerden toplu fatura oluşturun. Her hakediş için yalnızca bir fatura kesilebilir.
            </p>

            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300">Fatura Türü</label>
                <select x-model="bulk.invoice_type" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white">
            ...
            <x-ui.input type="date" label="Fatura Tarihi" x-model="bulk.invoice_date" />
            <x-ui.input type="number" step="1" min="0" max="100" label="KDV %" x-model="bulk.vat_rate" />
            ...
            <div x-show="bulkSaved" x-cloak class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-800/50 dark:bg-emerald-900/20 dark:text-emerald-300">
                Toplu fatura işlemi kaydedildi. e-Fatura/e-Arşiv entegrasyonu sonrası GİB'e otomatik gönderilecektir.
            </div>
