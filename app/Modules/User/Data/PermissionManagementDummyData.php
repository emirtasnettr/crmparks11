...
class PermissionManagementDummyData
{
    /** @var array<string, array<string, mixed>>|null */
    private static ?array $rolesPayloadCache = null;
     ...
     */
    public static function rolesPayload(): array
    {
        if (self::$rolesPayloadCache !== null) {
            return self::$rolesPayloadCache;
        }

        $payload = [];

        foreach (array_keys(self::selectableRoles()) as $roleSlug) {
            $defaults = self::defaultGrantsForRole($roleSlug);
            $payload[$roleSlug] = [
                'label' => self::selectableRoles()[$roleSlug],
                'is_locked' => $roleSlug === 'super_admin',
                'defaults' => $defaults,
                'matrix' => self::buildMatrix($defaults),
            ];
        }

        self::$rolesPayloadCache = $payload;
     ...
     */
    public static function summarize(string $roleSlug): array
    {
        $matrix = self::rolesPayload()[$roleSlug]['matrix'] ?? [];
        $applicable = 0;
        $active = 0;
    ...
    /**
     * Loglama mimarisi için örnek audit payload.
     *
     ...
     * @return array<int, string>
     */
    private static function defaultGrantsForRole(string $roleSlug): array
    {
        $all = self::allMatrixPermissionSlugs();

        $operational = self::slugsMatching([
            'dashboard.view',
            'business.', 'business_contact.', 'contract.', 'assignment.', 'earning.', 'document.',
            'courier.', 'agency.',
        ]);
        ...
        $userManagement = self::slugsMatching(['user.', 'role.', 'permission.']);
        $settings = self::slugsMatching(['setting.', 'activity_log.']);
    ...
    /**
     * Spatie Permission uyumlu modül tanımları.
     *
     * @return array<int, array<string, mixed>>
     */
