        ...
        FinanceCollectionPayment::query()
            ...
            ->each(function (FinanceCollectionPayment $payment) use (&$records, &$sequence): void {
                ...
                $records[] = [
                    ...
                    'current_account_name' => $collection?->business?->company_name ?? '—',
                    'description' => 'İşletme tahsilatı — '.($collection?->business?->company_name ?? '—'),
                    'amount_in' => $amount,
                    'amount_out' => 0.0,
        ...
        FinanceRevenue::query()
            ...
            ->each(function (FinanceRevenue $revenue) use (&$records, &$sequence): void {
                ...
                $records[] = [
                    ...
                    'current_account_name' => $revenue->business?->company_name ?? '—',
                    'description' => 'Gelir kaydı tahakkuku — '.($revenue->business?->company_name ?? '—'),
                    'amount_in' => $amount,
                    'amount_out' => 0.0,
